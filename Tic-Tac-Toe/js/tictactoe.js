...
//each player & determine which player won the roll
function rollForTurn() {
    ...
    diceRoll(); //play dice sound during game roll for turn
    //build the string to show which player rolled what die roll
    for (i=0; i<xArray.length; i++) {
        var result = i +1;
        var pOne = xArray[0];
        var pTwo = xArray[1];
        if (pOne == pTwo) { //rigging roll on tie to avoid bugin code...need to address this later
            pOne = 1;
            pTwo = 2;
        }
    ...
    if (pOne > pTwo) {
        first = "Player 1";
        setTimeout(function() { txt1 = txt1 + "Player 1 wins, please choose a square.";}, 2000); //setTimeout says do thisline of code then wait this amount of miliseconds before continuing
...
function startGame() {
    ...
    btnDisabled(btn); //disable the start buton since the game is now in progress
    var btn = document.getElementById("btnStop");
    stopEnabled(btn); //enable stop button since game is now in progress
...
//stop the current game and reset game when user indicates
function stopGame() {
    hideGameMsg(); //clear the text and hide msg box
    var btn = document.getElementById("btnStart");
    startEnabled(btn); //enable the start buton since game is now stopped
    var btn = document.getElementById("btnStop");
    ...
    //this clears the running log of the all the game moves
...
//this function will concel the message console from view
function hideGameMsg() {
    clearMsg() //clear the test from the msg console
    document.getElementById("gameMsgBox").style.display = "none"; //hide the div
}

//this function will write text to the game msg console
function writeMsg(txt) {
    showGameMsg();
    document.getElementById("gameMsg").innerHTML = txt;
}

//this function will clear the text from the msg console
function clearMsg() {
...
function determineAvatar() {
    ...
    var avatarArray = getAvatars(); //returns an array of both players assianged avatars
    var active = document.getElementById("showPlayer").innerHTML; //get active player
    p1Avatar = avatarArray[0];
    p2Avatar = avatarArray[1];
    if (active == "Player 1") { //checks which player is active and their corresponding avatar
...
function avatarPlaced() {
    ...
    //check if there is already a winner...if there is then don't continue game
    ...
    check4Tie(); //call this function to inquire if tie
}
...
//as square are selected they check in with this function to see of that particular 
//square has already been assigned and if it hasnt record new square w/ assigned avatar
function recordMoves(square) {
    var proposedMove = square;
    var boardState = document.getElementById("boardState").innerHTML; //retrive boardState array
...
//and then concatenate current move to it
...
function checkForWinCon() {
    var squareArray = [];
    var target = document.getElementById("boardState");
    var info = target.innerHTML; //raw array w/ squares and avatars
...
function glowBoard(pos) {
    ...
    for (var i=0; i<squares.length; i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg2.style.backgroundColor = "rgb(244, 179, 66)";}, 400);
...
function blink() {
    ...
    setTimeout(function() {body.style.backgroundColor = "#cc96f7";}, 500);
    setTimeout(function() {body.style.backgroundColor = "#d894f7";}, 600);
...
function checkWinCon1(info,squareArray) {
    ...
    // index0, index1, and index2; once they 
    // appear in the array, record their 
...
function checkWinCon4(info,squareArray) {
    ...
    winner(winDetected,winCon4); //winCon1 is the array of win combo
}
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { //if game has not yet started, prevent avatar placement
       ...
       if (verdict == undefined) { // if verdict is empty, then the square is unoccupied
           ...
           squareSound(); //play a game sound when the avatar is passed
       }
    }
}
